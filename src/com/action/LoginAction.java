...
public class LoginAction extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(method.equals("index")){//网页会员登陆
			String reg_user = Filter.escapeHTMLTags(request.getParameter("username").trim());
			String reg_pwd = Filter.escapeHTMLTags(request.getParameter("password").trim());
			String reg_type = "person";
			...
			if(flag == Constant.SUCCESS){//登陆成功
				...
				if(flag2 == Constant.SUCCESS){
					String type=new ComBean().getString("select sex from pmember where mid=(select id from member where username='"+reg_user+"')");
		...
		else if(method.equals("HEADUSERLOGIN")){//网页头部会员登陆
			...
			if(flag == Constant.SUCCESS){//登陆成功
				...
				if(flag2 == Constant.SUCCESS){
					String type=new ComBean().getString("select sex from pmember where mid=(select id from member where username='"+username+"')");
